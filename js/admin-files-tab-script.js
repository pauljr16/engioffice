// Global Variables
const FILES_ROOT_FOLDER = 'Office1Files';
...
function generateFolderButtonsHTML(folders, subFoldersData) {
  ...
  if (folders.length != 0) {
    ...
    folders.forEach( folder => {
      ...
      const folderButton = `
        <button class="btn btn-outline-secondary text-truncate file-category-button" type="button" onclick="openFolder('${folderFullPath}', '${firestoreDocName}', ${false})">
    ...
    folderButtonsHTML = `<h5 class="text-center mt-2">No files saved</h5>`;
  }
      ...
      <div class="col-6 col-sm-4 col-md-3 col-lg-3 custom-folder-col-xl px-1 mb-2 text-center file-and-foler-container" onclick="openFolder('${folderFullPath}', '${firestoreDocName}', ${true})">
        <div class="document-list border border-secondary">
          <a class="text-decoration-none" style="cursor: pointer;">
            ...
            <div class="${backgroundColor} ${textColor} mb-0 px-2 py-1 file-and-folder-title us-none">${displayName}</div>
          </a>
        </div>
      </div>
    `;
      ...
      <div class="col-6 col-sm-4 col-md-3 col-lg-3 custom-folder-col-xl px-1 mb-2 text-center file-and-foler-container" onclick="openFile('${fileFullPath}')">
        <div class="document-list border border-secondary">
          <a class="text-decoration-none">
            ...
            <p class="${backgroundColor} ${textColor} mb-0 px-2 py-1 file-and-folder-title us-none">${fileName}</p>
          </a>
        </div>
      </div>
    `;
...
function generateFolderNavigatorHTML(folderDisplayName) {
  ...
  if (CURRENT_FOLDER_PARENT_FOLDER === FILES_ROOT_FOLDER) {
    li = `<li class="breadcrumb-item us-none active" aria-current="page">${folderDisplayName}</li>`;
    ...
    for (let index = 0; index <= PREVIOUS_FOLDER_DISPLAY_NAME_LIST.length; index++) {
      if (index === PREVIOUS_FOLDER_DISPLAY_NAME_LIST.length) {
        li = `<li class="breadcrumb-item us-none active" aria-current="page">${CURRENT_FOLDER_DISPLAY_NAME}</li>`;
      } else {
        li = `<li class="breadcrumb-item us-none"><a class="breadcrumb-link" onclick="navigateFolder('${PREVIOUS_FOLDER_FULL_PATH_LIST[index]}', '${PREVIOUS_FOLDER_FIRESTORE_DOC_NAME_LIST[index]}', ${index})">${PREVIOUS_FOLDER_DISPLAY_NAME_LIST[index]}</a></li>`;
      }
